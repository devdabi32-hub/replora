...
const PLANS: Plan[] = [
  ...
  },
  {
    ...
    monthly: "₹999",
    annual: "₹9,990",
    monthlyLink: "https://rzp.io/rzp/3Km2lwAT",
    annualLink: "https://rzp.io/rzp/xjhNSYi",
    badge: "Most Popular",
    features: [
      "3 WhatsApp numbers",
      "Unlimited messages",
  ...
  },
  {
    ...
    monthly: "₹2,499",
    annual: "₹24,990",
    monthlyLink: "https://rzp.io/rzp/OwCF0wg",
    annualLink: "https://rzp.io/rzp/McR16MG",
    features: [
      "10 WhatsApp numbers",
  ...
  },
  {
    ...
    monthly: "₹4,999",
    annual: "₹49,990",
    monthlyLink: "https://rzp.io/rzp/2jdywV5",
    annualLink: "https://rzp.io/rzp/7QgNDmz8",
    features: [
      "25 WhatsApp numbers",
      "Everything in Pro",
      "PDF reports",
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10">
            ...
            <div className="mt-6 inline-flex items-center gap-2 p-1 rounded-full bg-white/[0.04] border border-white/10">
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-colors ${!annual ? "bg-white text-black" : "text-white/70"}`}
              >
                Monthly
              </button>
              <button
                onClick={() => setAnnual(true)}
                className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-colors flex items-center gap-2 ${annual ? "bg-white text-black" : "text-white/70"}`}
              >
                Annual
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#00c853]/20 text-[#00c853] border border-[#00c853]/30">
                  Save 2 months free
                </span>
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {PLANS.map((p) => {
              ...
              if (isOwner) {
                btn = (
                  <button disabled className="mt-5 h-10 w-full rounded-lg flex items-center justify-center gap-2 text-sm font-semibold bg-[#f59e0b]/15 text-[#f59e0b] border border-[#f59e0b]/30 cursor-not-allowed">
                ...
                if (isCurrent) {
                  ...
                  btn = (
                    <a href="https://wa.me/918989568529?text=Hi, I want to know about Replora Agency plan" target="_blank" rel="noopener noreferrer"
                      className="mt-5 h-10 w-full rounded-lg flex items-center justify-center gap-2 text-sm font-semibold text-white hover:opacity-90 transition-opacity"
                ...
                btn = (
                  <a href={link} target="_blank" rel="noopener noreferrer"
                    className="mt-5 h-10 w-full rounded-lg flex items-center justify-center gap-1.5 text-sm font-semibold text-white hover:opacity-90 transition-opacity"
                    style={{ backgroundColor: p.accent }}>
              ...
              return (
                <div key={p.key}
                  className="bg-[#0f1117] rounded-xl p-5 flex flex-col transition-all"
                  style={{ border: `1px solid ${p.badge ? "#0084ff" : "rgba(255,255,255,0.07)"}` }}>
                  <div className="flex items-center justify-between mb-3 min-h-[24px]">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-white/80">{p.name}</span>
                    {p.badge && (
                      <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border bg-[#0084ff]/15 text-[#0084ff] border-[#0084ff]/30">
                        {p.badge}
                      </span>
                    )}
                  </div>
                  <div className="text-2xl font-semibold text-white">
                    {priceLabel}
                    {priceSuffix && <span className="text-sm font-normal text-white/50">{priceSuffix}</span>}
                  </div>
          ...
          <div className="text-center mt-8 space-y-1">
            ...
            <p className="text-xs text-white/50">
              Need help or want to cancel? Email us at{" "}
              <a href="mailto:care@replora.in" className="text-[#0084ff] hover:underline">care@replora.in</a>
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {ADDONS.map((a) => {
              ...
              return (
                <div key={a.plan}
                  className="bg-[#0f1117] rounded-xl p-5 flex flex-col transition-all"
                  style={{ border: `1px solid ${highlight ? a.accent : "rgba(255,255,255,0.07)"}` }}>
                  <div className="text-[11px] font-bold uppercase tracking-wider text-white/80 mb-2">{a.name}</div>
                  <div className="text-2xl font-semibold text-white">
                    {a.price}<span className="text-sm font-normal text-white/50">/number/month</span>
                  </div>
                  <a href={a.link} target="_blank" rel="noopener noreferrer"
                    className={`mt-5 h-10 w-full rounded-lg flex items-center justify-center gap-1.5 text-sm font-semibold transition-opacity hover:opacity-90 ${isOwner ? "pointer-events-none opacity-50" : ""}`}
                    style={{ backgroundColor: a.accent, color: "white" }}>
                    Add number <ArrowUpRight className="h-4 w-4" />
                  </a>
                </div>
              );
            })}
          ...
          <p className="text-center text-xs text-white/50 mt-6">
            Addon numbers billed monthly. To cancel an addon contact{" "}
            <a href="mailto:care@replora.in" className="text-[#0084ff] hover:underline">care@replora.in</a>
          </p>
