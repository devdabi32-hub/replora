        ...
        <section id="pricing" className="py-28 px-6 lg:px-8 border-t border-white/[0.04]">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              {[
                { name: "Trial", price: "₹0", per: "/ 14 days", features: ["1 WhatsApp number", "Unlimited messages", "Full AI dashboard", "Hot/Warm/Cold lead tags"], highlight: false },
                { name: "Starter", price: "₹999", per: "/ month", features: ["3 WhatsApp numbers", "Unlimited messages", "AI dashboard", "30-day retention", "Email support"], highlight: true },
                { name: "Pro", price: "₹2,499", per: "/ month", features: ["10 WhatsApp numbers", "Everything in Starter", "Priority WhatsApp support", "60-day retention"], highlight: false },
                { name: "Growth", price: "₹4,999", per: "/ month", features: ["25 WhatsApp numbers", "Everything in Pro", "PDF reports", "90-day retention"], highlight: false },
                { name: "Agency", price: "Contact Sales", per: "", features: ["Unlimited numbers", "White label", "Mobile app", "Dedicated manager", "Custom SLA"], highlight: false },
              ].map((p) => (
                <div key={p.name} className={`relative rounded-2xl p-7 border transition-all ${p.highlight ? "border-[#0084ff]/60 bg-gradient-to-b from-[#0084ff]/[0.08] to-white/[0.02] shadow-2xl shadow-[#0084ff]/10" : "border-white/[0.08] bg-white/[0.02]"} backdrop-blur`}>
                  ...
                  <div className="mt-3 flex items-baseline gap-1.5">
                    <span className={`font-bold tracking-tight text-white ${p.per ? "text-3xl" : "text-xl"}`}>{p.price}</span>
                    {p.per && <span className="text-xs text-white/50">{p.per}</span>}
                  </div>
                  <Link to={p.name === "Agency" ? "/pricing" : "/signup"} className={`mt-6 w-full inline-flex items-center justify-center h-10 rounded-lg text-sm font-semibold transition-all ${p.highlight ? "bg-[#0084ff] hover:bg-[#0073e0] text-white shadow-lg shadow-[#0084ff]/30" : "bg-white/[0.06] hover:bg-white/[0.1] border border-white/10 text-white"}`}>
                    {p.name === "Agency" ? "Talk to us →" : "Start Free Trial"}
                  </Link>
